...
try:
        ...
        def _collect_object_members(self, src, node):
            ...
            for ch in node.children:
                if ch.type == "pair":
                    ...
                    if key is not None:
                        name = self._first_name_text(src, key) or self._node_text(
                            src, key
                        )
                        if name:
                            members.add(name)
                    ...
                    name = self._first_name_text(src, name) or self._node_text(
                        src, name
                    )
                    if name:
                        members.add(name)
                ...
                if var_name:
                    ...
                    if value_node is not None:
                        if value_node.type == "new_expression":
                            ...
                            ctor_name = self._first_name_text(
                                src, ctor
                            ) or self._node_text(src, ctor)
                            if ctor_name:
                                ctx["var_types"][var_name] = ctor_name
                        elif value_node.type == "object":
                            ctx["object_members"][var_name] = (
                                self._collect_object_members(src, value_node)
                            )
        ...
        def _current_class_name(self, tree, cursor_byte, src):
            ...
            while node is not None:
                if node.type == "class_declaration":
                    ...
                    return self._first_name_text(src, name) or self._node_text(
                        src, name
                    )
                node = node.parent
            return ""
            ...
            try:
                ...
                if receiver:
                    ...
                    if receiver in {"this", "super"} and current_class:
                        suggestions.update(
                            ctx["class_members"].get(current_class, set())
                        )
    ...
    lumos.plugin_manager._run_main_op(  # type: ignore
        "warning",
        "JavaScript Lexer",
